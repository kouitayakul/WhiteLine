...
function myFucntionAdd() {
  ...
  var tranType0 = ["Images", "GPS", "Checkbox", "Checklist", "Dropdown", "Number", "Radio", "Text", "Media Comment", "Text Comment"]
  // var db = openDatabase('mydb', '1.0', 'my first database', 1024 * 1024);
  //
  db.transaction(function(tx) {
  ...
  db.transaction(function(tx) {
    for (i = 0; i < tranType0.length; i++) {
      ...
      tx.executeSql('INSERT INTO receiptVFINAL (type, value) VALUES (?, ?)', [tranType0[i], y])
    ...
    tx.executeSql('SELECT SUM(value) as value FROM receiptVFINAL', [], function (tx, results) {
